...
use Clean/df_master, clear 
	...
	* Harmonizing pre & post-2013 income bracket 
	...
	/*
		label define salmet_labels 1 "€ < 500" 2 "500 <= € <1000" 3 "1000 <= € < 1250" 4"1250 <= € < 1500"
		5 "1500 <= € < 2000" 6 "2000 <= € < 2500" 7 "2500 <= 7 < 3000" 8 "3000 <= € < 5000" 9 "5000 <= €
		< 8000" 10 "8000 <= €" 98 "Refus" 99 "Ne sait pas"
...
// -----------------------
	...
	* PEPA Bonus Eligibility
	...
	/* 
		Sampled workers:  We have eliminated employees who work under the lump-sum-of-days regime, as 
		most managers do. For this category, it is not so much the weekly duration of work that is
		sensitive to detaxation as it is the total number of days worked during the year.
	*/
...
use Data/Clean/df2_master.dta, clear 
...
* Paid Overtime 
preserve 
	collapse (mean) emphre [pweight = new_weight], by(indiv_num datdeb_q)
	...
	twoway (bar emphre datdeb_q, barwidth(0.8)), ///
		   ...
		   ylabel(2(0.5) 6.5, grid labsize(small))

	graph export "Output/new1.1.paid_OT_overall.png", as(png) replace
restore

* Paid Overtime, by worker type
preserve
	keep if (stplc == 1 | stplc == 2)
	collapse (mean) emphre [pweight=new_weight], by(indiv_num datdeb_q stplc)           
	...
	graph export "Output/new1.2.paid_OT(bystplc).png", as(png) replace
restore 
...
preserve 
	...
	graph export "Output/new2.1.total_hours_worked_bystplc.png", as(png) replace 
restore 
...
preserve 
	...
	graph export "Output/new2.2.total_hours_worked_bystplc.png", as(png) replace 
restore 
...
preserve 
	keep if (stplc == 1 | stplc == 2)
	collapse (mean) hplus [pweight=new_weight], by (indiv_num datdeb_q stplc)
	...
	* Line Chart
	...
	graph export "Output/new3.2.desired_WH_line.png", as(png) replace
restore 
...
preserve	
	...
	collapse (count) valprie [pweight = new_weight], by(datdeb_q)
	...
	graph export "Output/new4.1.Quarterly_bonus_extensive_overall.png", as(png) replace
restore 
* By stplc
preserve 
	keep if (stplc == 1 | stplc == 2)
	collapse (count) valprie [pweight = new_weight], by(datdeb_q stplc) 
	...
	graph export "Output/new4.2Quarterly_bonuse_extensive_bystplc.png", as(png) replace
restore 

* Bonuses(intensive) - 2015 values 
* Overall 
preserve	
	collapse (mean) valprie_2015 [pweight = new_weight], by(datdeb_q)
	twoway (bar valprie datdeb_q, barwidth(0.8)), ///
		   xtitle("") ytitle("Bonus Amouts (2015 €)", size(small)) ///
		   xlabel(#20, format(%tq) angle(45) labsize(small)) ///
		   ylabel(, grid labsize(small))
		   
	graph export "Output/new5.1.Quarterly_bonus_intensive_overall.png", as(png) replace
restore 
* By stplc 
preserve 
	keep if (stplc == 1 | stplc == 2)
	collapse (mean) valprie_2015 [pweight = new_weight], by(datdeb_q stplc)
	...
	graph export "Output/new5.2.Quarterly_Bonus_intensive_bystplc.png", as(png) replace
restore 
...
gen new_weight = . 
		...
		replace new_weight = extri if annee >= 2015		

	preserve 
	
		/*
			Attention, la question portant sur le souhait de travailler davantage est 
			posée en toute généralité à partir de 2013 alors qu'elle portait auparavant
			sur la semaine de référence, ce qui a pour effet d'augmenter la propension 
			des enquêtés à y répondre positivement et en conséquence de rehausser le 
			niveau cumulé des indicateurs I et II du sous-emploi de 200 000 personnes 
			environ (consulter la variable SOUSEMPL). 
			
			BIG PROBLEM
		*/
		
		keep if (stplc == 1 | stplc == 2)
		replace stplc = 0 if stplc == 2
		collapse (mean) stplc [pweight = new_weight], by(datdeb_q)
		
		twoway (bar stplc datdeb_q, barwidth(0.8)), ///
						 xtitle("") ytitle("Percent of Motivated") xlabel(#20, format(%tq) angle(45) labsize(small)) ylabel(0.11(0.01) 0.2, labsize(small))
						 
	restore 
	
	preserve 
	
		replace stmn = 0 if stmn == 2 
		collapse (mean) stmn [pweight = new_weight], by(datdeb_q)
	
		twoway (bar stmn datdeb_q, barwidth(0.8)), ///
						 xtitle("") ytitle("Percent of Motivated") xlabel(#20, format(%tq) angle(45) labsize(small)) ylabel(, labsize(small))
...
////////////// Summary Stats //////////////
	...
	eststo sumstats: sum price mpg rep78 trunk, detail

	esttab sumstats using "Output/NAMEOFDOC.doc", ///
    cells("count(fmt(0)) mean(fmt(2)) p50(fmt(2)) sd(fmt(2)) min(fmt(2)) max(fmt(2))") ///
    label replace nomtitle ///
    title("INSERT TTILE") ///
    nonumber noobs ///
    varlabels(price "Colname" ///
              mpg   "Colname" ///
              rep78 "Colname" ///
			  trunk "Colname") 
...
////////////// Presenting Regressions //////////////
	...
	eststo model2: reg price mpg length 

	esttab model1 model2 using "Output/test.tex", ///
											  mtitles("COLNAME 1" "COLNAME 2") ///choose the column names 
											  r2 ar2 se nostar /// choose the stats you want 
...
twoway (lfitci price length, fintensity(10) lcolor()) (scatter price length, mcolor(midblue)), /// 
											title("Insert TITLE") ///
											xtitle("Insert X TITLE") ///
											ytitle("Insert Y TITLE") 
											legend(order(1 "Legend nme for command 1" 2 "Legend name for Command 2" ///
												         3 "legend name for the dots" ))
graph export "", as(png) replace 
														 
														 
hist price, by(foreign, row(1) /// FOR CONTINUOUS -- can do BY, not OVER 
									title("INSERT TITLE" ) note("")) ///
									xtitle("INSERT XTITLE") ///
									percent 
									xscale(range(0 17000)) xlabel(0(5000)17000, angle(45)) /// 
									yscale(range(0 60)) ylabel(0(10)60, angle()) 
									
									
graph bar //for categorical can do OVER not BY 
